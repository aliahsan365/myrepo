    ...
    if (v[i].consultar_mida() != 0 and not emparejados[i]) {
      ...
      while (j < tm and not intento) {
        if (not emparejados[j]) {
          ...
          } else if (mat[i][j] or v[j].consultar_mida() == 0)
            ++j;
        } else
          ++j;
      }
...
void Cjt_Organismo::aplica_estiron_conjunto(int x) {
  if (esta(x) and (v[x - 1].consultar_mida() != 0) and
      not v[x - 1].consultar_cortada()) {
...
int Cjt_Organismo::consultar_tamano_actual() const { return norg; }

bool Cjt_Organismo::esta(int x) const {
  return (x - 1) < consultar_tamano_actual();
}

int Cjt_Organismo::n_vivos() const { return nvivos; }

// LECTURA Y ESCRITURA

void Cjt_Organismo::escribir_organismos_nacidos_en_el_ultimo_emparejamiento(
    int hijos) {
