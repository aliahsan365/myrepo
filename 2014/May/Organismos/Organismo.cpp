...
bool Organismo::consultar_cortada() const { return cortada; }

int Organismo::consultar_mida() const { return mida; }

// MODIFICADORAS
// ESCRIBIR LA FUNCION DE  LA REPRODUCCION
void Organismo::aux_b(Arbre<Celula> &b, Arbre<Celula> &c, bool &act,
                      int &id_max, int &m, int &pid) {
  ...
  if (b1.es_buit() and b2.es_buit()) {
    ...
    bool act2 = false;
    if (not b1.es_buit())
      aux_b(b1, c1, act1, id_max, m, pid);
    if (not b2.es_buit())
      aux_b(b2, c2, act2, id_max, m, pid);
    Arbre<Celula> d1, d2;
...
void Organismo::aux_a(Arbre<Celula> &a, Arbre<Celula> &c, bool &act, int &m,
                      int &id_max) {
  ...
  if (a1.es_buit() and a2.es_buit()) {
    ...
    bool act2 = false;
    if (not a1.es_buit())
      aux_a(a1, c1, act1, m, id_max);
    if (not a2.es_buit())
      aux_a(a2, c2, act2, m, id_max);
    Arbre<Celula> d1, d2;
    if (not act1 and not act2) {
      if (raiz.activa) {
        if (raiz.id > id_max)
...
void Organismo::hacer_hijo(Arbre<Celula> &a, Arbre<Celula> &b, Arbre<Celula> &c,
                           int &id_max, int &m, int &pid) {
  Arbre<Celula> c1, c2;
  bool act = false;
...
void Organismo::aplica_estiron_organismo() { estiron(a, mida, id_maxim); }
...
void Organismo::escribir_organismo() { escribir_morfologia(a); }

void Organismo::leer_morfologia(Arbre<Celula> &a, int marca, int &m,
                                int &id_max) {
  Arbre<Celula> a1;
  Arbre<Celula> a2;
  Celula x;
...
void Organismo::leer_organismo() { leer_morfologia(a, 0, mida, id_maxim); }
