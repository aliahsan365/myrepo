...
void insert_ordered_element(struct list *list, struct elemento *e) {
  ...
  if (n == 0) { // lista vacia
  ...
  else { // lista ya contiene algun elemento (uno o mas)

    // reallocatamos memoria dinamica (en el heap) para (n+1) punteros al struct
    // elemento
    list->first = (struct elemento *)realloc(
        list->first, (n + 1) * sizeof(struct elemento *));
    ...
    if (pos == 0) { // hay algun elemento, pero nuestro nuevo elemento es el mas
                    // pequeño y, por lo tanto, va al princio
      ...
      while (aux != NULL) {
        if (pos !=
            0) { // seguimos avanzando hasta encontrar la pos donde insertar
          --pos;
          aux = aux->next;
        } else { // alcanzamos la pos correcta para insertar
          struct elemento *temp = aux->next;
          if (temp == NULL) { // nuestro el elemento e es el actual mayor
                              // absoluto (solitario) de la lista
          ...
          else { // vamos a insertar entre dos elementos, entre aux y temp
...
void insert_ordered_element2(struct list *list, struct elemento *e) {
  ...
  else { // lista ya contiene algun elemento
