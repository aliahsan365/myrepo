...
//Key idea: To use both unidimensional array position  and content to represent tower position (i,j) on a chessboard.
...
//A solution is only acceptable if and only if there is no repetitions within the unidimensional array.
//This means, no two towers on the same row.
bool check_constraint_all_different(const vector<int>& sol) {
    int N = sol.size();
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            // DO NOT  compare with itself. Obiously, we allow to be the same in this case.
            if (i != j) {
                if (sol[i] == sol[j])
                    return false;
            }
...
//Pure computation of ALL permutations, using backtracking, with no heuristics and no extra structures. 
